...
public class MultipleChoiceQuestionDAO implements QuestionDAO, AnswerDAO {
    ...
    public void addAnswer(ArrayList<Answer> answers, long questionId) throws SQLException {
        for (Answer ans : answers) {
            PreparedStatement stm = myConn.prepareStatement("INSERT INTO multipleChoiceAnswers (answer_text, question_id, is_correct) VALUES (?, ?, ?)", PreparedStatement.RETURN_GENERATED_KEYS);
    ...
    public List<Answer> getAnswer(long questionId) throws SQLException {
        ...
        while (res.next()) {
            lst.add(new Answer(res.getLong("id"), res.getString("answer_text"), res.getLong("question_id"), res.getBoolean("is_correct")));
        }
        return lst;
    }
    ...
    public void addQuestion(Question question, long quizId) throws SQLException {
        PreparedStatement stm = myConn.prepareStatement("INSERT INTO multipleChoiceQuestions (question_text, quiz_id, num_of_answers) VALUES (?, ?, ?)", PreparedStatement.RETURN_GENERATED_KEYS);
